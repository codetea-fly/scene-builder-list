...
function Hero() {
  ...
  return (
    <section className="relative isolate flex min-h-[calc(100vh-4rem)] flex-col overflow-hidden border-b border-sky-100">
      <HeroPointCloud />
      <div className="absolute right-3 top-3 z-10 max-w-[14rem] animate-fade-in text-right sm:right-8 sm:top-6 sm:max-w-[18rem]"
        style={{ animationDelay: "320ms", animationFillMode: "backwards" }}>
        <div className="mb-1 flex items-center justify-end gap-1.5 text-[10px] font-medium tracking-wider text-slate-400/80 sm:mb-1.5 sm:gap-2 sm:text-xs">
          <span className="h-px w-4 bg-slate-200/70 sm:w-6" />
          指导单位
        </div>
        <div className="space-y-0.5 text-[9px] font-normal leading-snug text-slate-500/70 sm:text-xs sm:leading-relaxed">
          <p>中国信息通信研究院</p>
          ...
          {SERVICES.map((s, i) => (
            <Reveal key={s.title} variant="slide-right" delay={i * 130} duration={900}>
              <Link to="/promotion/lifecycle"
                className="group relative flex flex-col overflow-hidden rounded-2xl border border-sky-100 bg-white/80 px-6 py-12 shadow-sm backdrop-blur transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-sky-200/50 min-h-[360px]">
                <div className="mb-6 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-sky-500 to-blue-600 shadow-md shadow-sky-300/50 transition-transform group-hover:scale-110">
                ...
                <p className="mt-4 text-sm leading-relaxed text-slate-500">{s.desc}</p>
