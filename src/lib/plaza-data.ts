...
export const SCENES: Scene[] = RAW.map((s, i) => {
  ...
  return {
    ...
    images: [
      { caption: "孪生主视图", gradient: IMG_GRADIENTS[(i + 0) % IMG_GRADIENTS.length], image: SCENE_IMAGES[i % SCENE_IMAGES.length] },
    ...
    videos: [
      { title: "场景概览短片", duration: "01:32", gradient: IMG_GRADIENTS[(i + 1) % IMG_GRADIENTS.length], image: SCENE_IMAGES[i % SCENE_IMAGES.length] },
